...
import com.capstone.merkado.DataManager.DataFunctions.MarketError;
...
public class StoreConsumerView extends AppCompatActivity {
    ...
    ImageView itemImage, itemResourceGroup;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        itemResourceGroup = findViewById(R.id.item_resource_group);
        ...
        playerMarketUpdates = new PlayerMarketUpdates(merkado.getPlayer().getServer(), playerMarkets.getMarketOwner());
        playerMarketUpdates.startListener(pm -> {
            playerMarkets = pm;
            setUpView(pm.getOnSale(), currentDisplayMode);
        });
        storeShowCollectibles.setOnClickListener(v -> currentDisplayMode = DisplayMode.COLLECTIBLES);
        storeShowEdibles.setOnClickListener(v -> currentDisplayMode = DisplayMode.EDIBLES);
        storeShowResources.setOnClickListener(v -> currentDisplayMode = DisplayMode.RESOURCES);
    ...
    private void setUpView(List<OnSale> onSaleList, DisplayMode displayMode) {
        ...
        storeViewConsumerAdapter = new StoreViewConsumerAdapter(this, filterSaleList(onSaleList, displayMode));
        itemList.setAdapter(storeViewConsumerAdapter);
        storeViewConsumerAdapter.setOnClickListener(this::setUpDetails);
        setUpDetails(onSaleList.get(0), 0);
    }
        ...
        switch (displayMode) {
            case COLLECTIBLES:
                return onSaleList.stream()
                        .filter(onSale -> "COLLECTIBLES".equals(onSale.getType()))
                        .collect(Collectors.toList());
            case EDIBLES:
                return onSaleList.stream()
                        .filter(onSale -> "EDIBLES".equals(onSale.getType()))
                        .collect(Collectors.toList());
            case RESOURCES:
                return onSaleList.stream()
                        .filter(onSale -> "RESOURCES".equals(onSale.getType()))
                        .collect(Collectors.toList());
            default:
    ...
    private void setUpDetails(OnSale onSale, Integer onSaleId) {
        ...
        int itemTypeResource = GameResourceCaller.getResourceTypeIcons(onSale.getType());
        ...
        itemResourceGroup.setImageDrawable(ContextCompat.getDrawable(getApplicationContext(), itemTypeResource));
